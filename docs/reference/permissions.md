...
| `destructive`      | Delete things outright — with only the write grants, it can add and change but never remove.<br>**Careful:** What this deletes is gone. Leave it off unless you want the token removing things, not just adding and changing them. | `cancel_block`, `drop_todo`, `remove_notebook`, `remove_idea`, `remove_inventory_item`, `remove_inventory_category`, `remove_goal_target`, `cancel_alarm`, `remove_repeating_block`, `remove_location`, `remove_workout_session`, `remove_workout_category`, `delete_sort_rule`                                                                                         |
