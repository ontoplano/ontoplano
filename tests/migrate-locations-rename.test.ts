...
describe('0055, on an account that already had an inventory', () => {
	...
	/*
	 * The whole answer to "will this preserve my shopping list": nothing is
	 * defaulted into a location, because a row with none is exactly a
	 * shopping-list line and always was.
	 */
	test('and a plain shopping-list line is untouched, with no address invented for it', () => {
		expect(one('select name, location_id, bought from shopping_items where id = 2')).toEqual({
			name: 'Milk',
			location_id: null,
			bought: 0
		});
