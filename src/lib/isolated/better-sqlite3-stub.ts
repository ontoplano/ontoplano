 ...
 * client — a path the isolated instance never takes, because the worker builds
	...
	constructor() {
		throw new Error(
			'better-sqlite3 does not exist in a browser. ' +
				'Open the WASM database and hand drizzle() the client from $lib/isolated/wasm-client.'
		);
	}
}
