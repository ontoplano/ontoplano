/**
 * The local instance's answer to the app's own requests.
 *
 * SvelteKit's client asks a server for two things: `__data.json` when it
 * loads a page, and a `?/action` POST when a form submits. In local mode this
 ...
 * server, and no route, form, or component changes to run locally. A request
 * for anything else, or for a route with no local twin, goes to the network
...
export function installLocalBridge(): void {
	if (installed) return;
	installed = true;
