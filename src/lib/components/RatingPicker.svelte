...
<div class={compact ? 'flex items-center gap-2' : 'space-y-1'}>
	<div class={compact ? 'w-16 shrink-0' : 'flex items-baseline justify-between'}>
		<span class="eyebrow text-gray-500">{RATING_LABELS[rating]}</span>
		{#if !compact}
			<span class="text-xs text-gray-400">{RATING_HINTS[rating]}</span>
		{/if}
	</div>

	<div class="flex items-center gap-1">
		{#if !compact}
			<span class="w-16 shrink-0 text-right text-[10px] text-gray-400">{low}</span>
		{/if}
		{#each steps as n (n)}
			<button
				type="button"
				...
				class="tabular h-6 w-6 border text-xs {value !== null && n <= value
					...
					: 'border-gray-300 bg-white text-gray-400 hover:border-gray-500 hover:text-gray-700'}"
			>
				{n}
			</button>
		{/each}
		{#if !compact}
			<span class="w-16 shrink-0 text-[10px] text-gray-400">{high}</span>
		{/if}
		{#if value !== null}
			<button
				type="button"
				onclick={() => (value = null)}
				class="ml-1 text-xs text-gray-400 hover:text-gray-900"
