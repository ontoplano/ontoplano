<script lang="ts">
	...
	import type { FlowPageData } from '../../routes/finance/flows';

	/**
	 * Every user-facing string that differs between the two directions.
	 * The mechanism — form, payments, archive, delete — is identical, which
	 * is the point of this being one component.
	 */
	type Wording = {
		/** 'New bill' / 'New income' */
		newThing: string;
		...
		/** The chip on a settled row: 'paid' / 'received'. */
		paidChip: string;
		/** 'Mark paid' / 'Mark received' */
		markPaid: string;
		/** The little confirm button: 'Paid' / 'Received'. */
		paidButton: string;
		paidSoFar: string;
	};
...
<div class="space-y-5">
	...
	<div class="flex flex-wrap items-baseline gap-x-6 gap-y-1 rounded border border-gray-200 p-4">
		...
		<div>
			<div class="text-xs text-gray-500">{wording.paidSoFar}</div>
	...
	{#if active.length === 0}
		<EmptyState icon="wallet" title={wording.emptyTitle} description={wording.emptyDescription} />
	{:else}
					...
					<div class="flex flex-1 flex-wrap items-center justify-end gap-2">
						...
						{:else if paying === bill.id}
							...
							>
								...
								/>
								<button class="btn btn-primary btn-sm" type="submit">{wording.paidButton}</button>
								<button class="btn btn-sm" type="button" onclick={() => (paying = null)}>×</button>
							</form>
						{:else}
							<button
								class="icon-btn"
								title={wording.markPaid}
								aria-label="Mark {bill.name} {wording.paidChip}"
...
<Modal
	bind:open={showForm}
	error={form?.message}
	title={editing ? wording.editTitle : wording.newThing}
...
<Modal
	...
	title={wording.deleteTitle}
	onclose={() => (confirmingDelete = null)}
	size="sm"
>
	{#if confirmingDelete}
		<p class="text-sm text-gray-600">
			<strong>{confirmingDelete.name}</strong>{wording.deleteWarning}
		</p>
	{/if}
	{#snippet footer()}
