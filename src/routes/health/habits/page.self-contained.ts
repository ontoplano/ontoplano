...
export const actions = {
	...
	logOccurrence: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	toggleOccurrence: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	updateOccurrence: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			updateOccurrence(
	...
	deleteOccurrence: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
