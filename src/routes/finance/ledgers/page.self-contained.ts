...
export const actions = {
	createLedger: async ({ request, locals }: SelfContainedEvent) => {
	...
	updateLedger: async ({ request, locals }: SelfContainedEvent) => {
		const form = await request.formData();
		try {
	...
	moveLedger: async ({ request, locals }: SelfContainedEvent) => {
		const form = await request.formData();
		try {
	...
	archiveLedger: async ({ request, locals }: SelfContainedEvent) => {
		const form = await request.formData();
		try {
			setLedgerArchived(
	...
	deleteLedger: async ({ request, locals }: SelfContainedEvent) => {
		const form = await request.formData();
		try {
	...
	addMovement: async ({ request, locals }: SelfContainedEvent) => {
		const form = await request.formData();
		const ctx = buildCtx(locals.user!.id);
		try {
	...
	updateMovement: async ({ request, locals }: SelfContainedEvent) => {
		const form = await request.formData();
		const ctx = buildCtx(locals.user!.id);
		try {
