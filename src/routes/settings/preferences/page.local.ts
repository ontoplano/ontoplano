...
export const load = async ({ locals }: LocalRouteEvent) => {
	...
	return {
		// The local answer. There is nowhere to send a report from a device's
...
export const actions = {
	saveCurrency: async ({ request, locals }: LocalRouteEvent) => {
	...
	saveGridHours: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	 ...
	 */
	saveMenu: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
	...
	resetMenu: async ({ locals }: LocalRouteEvent) => {
		const userId = locals.user!.id;
		setNavOrder(userId, []);
	...
	setLayout: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
	...
	resetLayout: async ({ locals }: LocalRouteEvent) => {
		setUserSetting(locals.user!.id, DASHBOARD_LAYOUT_KEY, serialiseLayout(defaultLayout()));
		return { success: true, action: 'setLayout' };
	},

	addQuote: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
	...
	importQuotes: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
	...
	deleteQuote: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
	...
	setStyle: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
	...
	setTheme: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
	...
	saveWeek: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
