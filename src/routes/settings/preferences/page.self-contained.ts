...
export const load = async ({ locals }: SelfContainedEvent) => {
	...
	return {
		// The self-contained answer. There is nowhere to send a report from a device's
		// own instance, and 'off' is what hides the row; the server overrides
		// this with the account's real state in +page.server.ts.
		errorReports: 'off' as const,
...
export const actions = {
	saveCurrency: async ({ request, locals }: SelfContainedEvent) => {
	...
	saveGridHours: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	 ...
	 */
	saveMenu: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
	...
	resetMenu: async ({ locals }: SelfContainedEvent) => {
		const userId = locals.user!.id;
		setNavOrder(userId, []);
	...
	setLayout: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
	...
	resetLayout: async ({ locals }: SelfContainedEvent) => {
		setUserSetting(locals.user!.id, DASHBOARD_LAYOUT_KEY, serialiseLayout(defaultLayout()));
		return { success: true, action: 'setLayout' };
	},

	addQuote: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
	...
	importQuotes: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
	...
	deleteQuote: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
	...
	setStyle: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
	...
	setTheme: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
	...
	saveWeek: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
