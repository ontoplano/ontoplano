 ...
 * reaches the bytes — see `$lib/self-contained/pictures.ts`.
 */
...
export const actions = {
	 ...
	 */
	planFolder: async ({ request, locals }: SelfContainedEvent) => {
		const form = await request.formData();
		try {
	 ...
	 */
	importFolder: async ({ request, locals }: SelfContainedEvent) => {
		const form = await request.formData();
		try {
