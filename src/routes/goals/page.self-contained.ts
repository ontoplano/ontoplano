...
export const actions = {
	 ...
	 */
	setTodoStatus: async ({ request, locals }: SelfContainedEvent) => {
	...
	createArea: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	deleteArea: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	setProgress: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			setTargetProgress(
				buildCtx(locals.user!.id),
	...
	close: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	setLinks: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	remove: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
