...
export const actions = {
	 ...
	 */
	reorder: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	 ...
	 */
	promote: async ({ request, locals }: LocalRouteEvent) => {
	...
	demote: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	createTodo: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	setRatings: async ({ request, locals }: LocalRouteEvent) => {
		const ctx = buildCtx(locals.user!.id);
		const target = await readTarget(request);
		if ('error' in target) return fail(400, { message: target.error });
	...
	remind: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	unremind: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	editInstance: async ({ request, locals }: LocalRouteEvent) => {
		const ctx = buildCtx(locals.user!.id);
		const formData = await request.formData();
	...
	resolveActivity: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
			resolveInstanceActivity(
	...
	deleteInstance: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	deleteTodo: async ({ request, locals }: LocalRouteEvent) => {
		const target = await readTarget(request);
		if ('error' in target) return fail(400, { message: target.error });
