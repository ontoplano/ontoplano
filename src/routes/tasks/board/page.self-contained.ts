...
export const actions = {
	 ...
	 */
	reorder: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	schedule: async ({ request, locals }: SelfContainedEvent) => {
		const target = await readTarget(request);
		if ('error' in target) return fail(400, { message: target.error });
		if (target.kind !== 'todo')
	 ...
	 */
	promote: async ({ request, locals }: SelfContainedEvent) => {
	...
	demote: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	createTodo: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	setRatings: async ({ request, locals }: SelfContainedEvent) => {
		const ctx = buildCtx(locals.user!.id);
		const target = await readTarget(request);
		if ('error' in target) return fail(400, { message: target.error });
	...
	remind: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	unremind: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	editInstance: async ({ request, locals }: SelfContainedEvent) => {
		const ctx = buildCtx(locals.user!.id);
		const formData = await request.formData();
	...
	resolveActivity: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			resolveInstanceActivity(
	...
	deleteInstance: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	deleteTodo: async ({ request, locals }: SelfContainedEvent) => {
		const target = await readTarget(request);
		if ('error' in target) return fail(400, { message: target.error });
