...
export const load = async ({ locals, url, cookies }: IsolatedEvent) => {
...
export const actions = {
	 ...
	 */
	unscheduleBlock: async ({ request, locals }: IsolatedEvent) => {
		const formData = await request.formData();
	...
	updateExceptional: async ({ request, locals }: IsolatedEvent) => {
		const ctx: Ctx = buildCtx(locals.user!.id);
		const formData = await request.formData();
