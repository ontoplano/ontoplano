...
export const load = async ({ locals, url, cookies }: SelfContainedEvent) => {
...
export const actions = {
	...
	bulkDelete: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	copyToWeekdays: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			copySlotsToWeekdays(
	...
	addCalendar: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	removeCalendar: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	applyTemplate: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			applyTemplate(
				buildCtx(locals.user!.id),
	...
	saveScheme: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	loadScheme: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	deleteScheme: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	renameScheme: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			renameScheme(
				buildCtx(locals.user!.id),
	...
	scheduleTodo: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
	 ...
	 */
	unscheduleBlock: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
	...
	convertRepeat: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	moveOccurrence: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	 ...
	 */
	setStatus: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			setStatusOn(
				buildCtx(locals.user!.id),
	...
	suppress: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			suppressOccurrence(
				buildCtx(locals.user!.id),
	...
	unsuppress: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			unsuppressOccurrence(
				buildCtx(locals.user!.id),
	...
	createExceptional: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
	...
	updateExceptional: async ({ request, locals }: SelfContainedEvent) => {
		const ctx: Ctx = buildCtx(locals.user!.id);
		const formData = await request.formData();
	...
	deleteExceptional: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	importCsv: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
