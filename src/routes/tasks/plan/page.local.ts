...
export const load = async ({ locals, url, cookies }: LocalRouteEvent) => {
...
export const actions = {
	...
	toggleActive: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	bulkDelete: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	copyToWeekdays: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
			copySlotsToWeekdays(
	...
	addCalendar: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	removeCalendar: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	applyTemplate: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
			applyTemplate(
				buildCtx(locals.user!.id),
	...
	saveScheme: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	loadScheme: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	deleteScheme: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	renameScheme: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
			renameScheme(
				buildCtx(locals.user!.id),
	...
	scheduleTodo: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
	 ...
	 */
	unscheduleBlock: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
	...
	convertRepeat: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	moveOccurrence: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	suppress: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
			suppressOccurrence(
				buildCtx(locals.user!.id),
	...
	unsuppress: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
			unsuppressOccurrence(
				buildCtx(locals.user!.id),
	...
	createExceptional: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
	...
	updateExceptional: async ({ request, locals }: LocalRouteEvent) => {
		const ctx: Ctx = buildCtx(locals.user!.id);
		const formData = await request.formData();
	...
	deleteExceptional: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	importCsv: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
