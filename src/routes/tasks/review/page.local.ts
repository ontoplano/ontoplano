...
export const actions = {
	saveNote: async ({ request, locals }: LocalRouteEvent) => {
	...
	keepStale: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
			keepStale(
	...
	completeStale: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
			completeStale(
				buildCtx(locals.user!.id),
	...
	dropStale: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
			dropStale(
				buildCtx(locals.user!.id),
	...
	resolve: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	 ...
	 */
	settle: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
	...
	carry: async ({ request, locals }: LocalRouteEvent) => {
		const formData = await request.formData();
		try {
