...
export const actions = {
	saveNote: async ({ request, locals }: SelfContainedEvent) => {
	...
	keepStale: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			keepStale(
	...
	completeStale: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			completeStale(
				buildCtx(locals.user!.id),
	...
	dropStale: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			dropStale(
				buildCtx(locals.user!.id),
	...
	resolve: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	 ...
	 */
	settle: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	carry: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
