...
export const actions = {
	...
	toggleApplied: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			toggleApplied(
	...
	updateAppliedNote: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			updateAppliedNote(
				buildCtx(locals.user!.id),
	...
	toggleFavorite: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
