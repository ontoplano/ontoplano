...
export const actions = {
	...
	setCategoryFood: async ({ request, locals }: SelfContainedEvent) => {
	...
	setCategoryShared: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			setCategoryShared(
				buildCtx(locals.user!.id),
	...
	renameCategory: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	saveCategories: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		const ctx = buildCtx(locals.user!.id);
	 ...
	 */
	setQty: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	toggleBought: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	paid: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	restock: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	toggleSnoozed: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	createLocation: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	updateLocation: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	...
	deleteLocation: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
	 ...
	 */
	putItem: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		const raw = String(formData.get('locationId') ?? '');
		try {
			setItemLocation(
	...
	setFields: async ({ request, locals }: SelfContainedEvent) => {
		const formData = await request.formData();
		try {
			setItemAttributes(
				buildCtx(locals.user!.id),
