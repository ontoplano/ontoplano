...
function apiPage() {
	 ...
	 */
	// An endpoint that also runs on an isolated instance keeps its handlers — and
	// its prose — in endpoint.portable.ts, with +server.ts a bare re-export. Read
	// the file the logic actually lives in.
	const locals = new Set(walk(join(ROOT, 'src/routes'), (f) => f === 'endpoint.portable.ts'));
	const files = walk(join(ROOT, 'src/routes'), (f) => f === '+server.ts').map((f) => {
		const local = join(dirname(f), 'endpoint.portable.ts');
...
function pagesPage() {
	// A route that also runs on an isolated instance keeps its bodies — and its
	// prose — in page.portable.ts, with +page.server.ts re-exporting it. Both
	...
	const locals = new Set(walk(join(ROOT, 'src/routes'), (f) => f === 'page.portable.ts'));
	const serverFiles = walk(join(ROOT, 'src/routes'), (f) => f === '+page.server.ts').flatMap(
		(f) => {
			const local = join(dirname(f), 'page.portable.ts');
...
const FRAGMENTS = {
	...
	downloads: () => {
		...
		return [
			...
			`curl -LO ${url(names.pkgbuild)}`,
			`curl -LO ${url(names.install)}`,
			'makepkg -si',
			'```',
			'',
			...
			'ones you did not.',
			'',
			`The recipe is covered twice over: \`${names.sums}\` covers the \`${names.pkgbuild}\` you`,
